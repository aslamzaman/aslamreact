...
export const getDataFromFirebase = async (collectionName) => {
    try {
        ...
        const maxCacheAge = 2 * 60 * 60 * 1000; // 1 hour
        // const maxCacheAge = 60 * 1000; // 1 hour
...
export const getDataFromFirebase1 = async (collectionName) => {
    try {
        ...
        if (localStorageData) {
            ...
            if (localData.timestamp && now - localData.timestamp < maxCacheAge) {
                console.log("Data from localStorage");
...
export const addDataToFirebase = async (collectionName, data) => {
    try {
        ...
        if (cachedData) {
            try {
                jsonCacheData = JSON.parse(cachedData);
                if (!Array.isArray(jsonCacheData)) {
                    jsonCacheData = [];
                }
            } catch (error) {
        ...
        jsonCacheData.push({ id: docRef.id, ...data });
        localStorage.setItem(collectionName, JSON.stringify(jsonCacheData));
...
export const addDataToFirebase1 = async (collectionName, data) => {
    try {
        const collectionRef = collection(db, collectionName);
        const docRef = await addDoc(collectionRef, data);
       
        const cachedData = JSON.parse(localStorage.getItem(collectionName)) || { data: [], timestamp: Date.now() };
        let localData = cachedData.data;

        localStorage.setItem(collectionName, JSON.stringify({
            data: [...localData, { id: docRef.id, ...data }],
...
export const updateDataToFirebase = async (collectionName, id, data) => {
    try {
        ...
        const cachedData = localStorage.getItem(collectionName);
        if (cachedData) {
            let jsonCacheData = JSON.parse(cachedData);
            let existingData = jsonCacheData.data;
            const indexOfItem = jsonCacheData.findIndex(item => item.id === id);


            if (indexOfItem !== -1) {
                jsonCacheData[indexOfItem] = { id, ...data };
                localStorage.setItem(collectionName, JSON.stringify(jsonCacheData));
            } else {
...
export const updateDataToFirebase1 = async (collectionName, id, data) => {
    try {
        const collectionRef = collection(db, collectionName);
        const refDoc = doc(collectionRef, id);
        await setDoc(refDoc, data);
        ...
        if (cachedData) {
            let jsonCacheData = JSON.parse(cachedData);
            let existingData = jsonCacheData.data;
            const indexOfItem = existingData.findIndex(item => item.id === id);


            if (indexOfItem !== -1) {
                existingData[indexOfItem] = { id, ...data };
                localStorage.setItem(collectionName, JSON.stringify(jsonCacheData));

                localStorage.setItem(collectionName, JSON.stringify({
                    data: existingData,
                    timestamp: jsonCacheData.timestamp
                }));
...
export const updateDataToFirebase2 = async (collectionName, id, data) => {
    try {
        const collectionRef = collection(db, collectionName);
        const refDoc = doc(collectionRef, id);
        await setDoc(refDoc, data);
        ...
        const cachedData = JSON.parse(localStorage.getItem(collectionName)) || { data: [], timestamp: Date.now() };
        let localData = cachedData.data;
        const indexOfItem = localData.findIndex(item => item.id === id);

        if (indexOfItem !== -1) {
            localData[indexOfItem] = { id, ...data };
        } else {
...
export const deleteDataFromFirebase = async (collectionName, id) => {
    try {
        ...
        if (cachedData) {
            let jsonCacheData = JSON.parse(cachedData);
            const notDeletedItems = jsonCacheData.filter(item => item.id !== id);
            localStorage.setItem(collectionName, JSON.stringify(notDeletedItems));
        } else {
            console.log("No cached data found!")
        }
...
export const deleteDataFromFirebase1 = async (collectionName, id) => {
    try {
        const collectionRef = collection(db, collectionName);
        const refDoc = doc(collectionRef, id);
        await deleteDoc(refDoc);
        ...
        if (cachedData) {
            let jsonCacheData = JSON.parse(cachedData);
            let localData = jsonCacheData.data;
            const notDeletedItems = localData.filter(item => item.id !== id);

            localStorage.setItem(collectionName, JSON.stringify({
                data: notDeletedItems,
                timestamp: jsonCacheData.timestamp
            }));
