...
const Print = ({ Msg }) => {
	...
	const printHandler = async () => {
		alert("No results found!"); return false;
		const doc = new jsPDF({
			orientation: "p",
			unit: "mm",
			format: "a4",
		...
		const author = await fetchAll("author");

		let y = 20;
		for (let i = 0; i < author.length; i++) {
			doc.text(`${author[i].name}`, 30, y, null, null, "center");
			doc.text(`${author[i].deg}`, 60, y, null, null, "center");
			y = y + 6;
		}

		doc.save(`${Date.now()}-author.pdf`);
		Msg("Print completed.");
		setShow(false);
	}
