...
const Edit = ({ Msg, Id }) => {
	...
	const editHandler = async () => {
		...
		Msg("Ready to edit");
		try {
			const placeData = await fetchOne("place", Id);
			if (placeData) {
	...
	const saveHandler = async (e) => {
		...
		try {
			...
			const updatedPlaceId = await updateOne("place", placeData);
			console.log(`Place with id ${updatedPlaceId} updated successfully.`);
			Msg("Data updated successfully.");
		} catch (error) {
			console.log(`Error updating place data: ${error}`);
					...
					<div className="px-6 pb-6 text-black">
						<form onSubmit={saveHandler} >
							<div className="grid grid-cols-1 gap-4 my-4">
								<TextEn Title="Name(SC/Field)" Id="name" Change={e => setName(e.target.value)} Value={name} Chr="50" />
							</div>
