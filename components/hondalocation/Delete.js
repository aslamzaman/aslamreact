...
import { deleteOne } from "../DexieDatabase";
...
const Delete = ({ Msg, Id }) => {
	...
	const removeHandler = async () => {
		try {
			await deleteOne("hondalocation", Id);
			Msg("Data deleted successfully.");
		} catch (error) {
			console.log(`Error deleting data: ${error}`);
			Msg("Data deleting error");
		}
		setShow(false);
	}
	...
	return (
		<>
			...
			<button onClick={handleDelete} title="Delete" className="w-8 h-8 rounded-full hover:bg-gray-200 mr-2 flex justify-center items-center">
