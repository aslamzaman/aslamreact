...
const Edit = ({ message, id, data }) => {
    ...
    const showEditForm = () => {
           setShow(true);
            const {dt, unit, title, cat, picurl, createdAt} = data;
            setDt(dt);
            setUnit(unit);
            setTitle(title);
            setCat(cat);
    ...
    const saveHandler = async (e) => {
        ...
        try {
            ...
            const msg = await updateDataToFirebase("doc",id, newObject);
            message(msg);
        } catch (error) {
                        ...
                        <div className="px-6 pb-6 text-black">
                            <form onSubmit={saveHandler} >
                                <div className="grid grid-cols-1 gap-4 my-4">
                                    ...
                                    <TextEn Title="Title" Id="title" Change={e => setTitle(e.target.value)} Value={title} Chr={50} />
                                    <TextEn Title="Cat" Id="cat" Change={e => setCat(e.target.value)} Value={cat} Chr={50} />
