...
const EditPage = (tbl, fld) => {
  ...
  str = str + 'const Edit = ({Data, Msg, Id}) => {\n';
  let a = "";
  ...
  str = str + 'const ' + tbl + ' = Data.find((t) => parseInt(t.id) === parseInt(Id));\n';
 ...
 str = str + '};\n\n\n';
  ...
  str = str + 'const updateExisting' + titleCase(tbl) + ' = async (' + tbl + ') => {\n';
  str = str + 'const updateMsg = await update' + titleCase(tbl) + '(' + tbl + ');\n';
  str = str + 'if (updateMsg.success) {\n';
  str = str + 'return "' + titleCase(tbl) + ' information successfully updated.";\n';
  str = str + '} else {\n';
  str = str + 'throw new Error("Error: Failed to update ' + tbl + ' information.");\n';
  str = str + '}\n';
  str = str + "}\n\n";
  ...
  str = str + 'const indexToUpdate = Data.findIndex((data) => parseInt(data.id) === parseInt(Id));\n';
  str = str + 'if (indexToUpdate === -1) {\n';
  str = str + 'throw new Error("Error: ' + titleCase(tbl) + ' information not found.");\n';
  str = str + '}\n';
  str = str + 'Data[indexToUpdate] = ' + tbl + ';\n';
  str = str + 'const message = await updateExisting' + titleCase(tbl) + '(Data);\n';
  str = str + ' Msg(message);\n';
  str = str + '} catch (error) {\n';
  str = str + 'console.log(error);\n';
  str = str + 'Msg("Error: Failed to update ' + tbl + ' information.");\n';
  str = str + '}\n';
