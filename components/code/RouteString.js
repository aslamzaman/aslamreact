const RouteString = (tbl, fld) => {
	...
	str = str + 'const BackupRestore = require("../library/BackupRestore");\n';
	...
	for (i = 0; i < splitFld.length - 1; i++) {
		...
		arr = arr + '"' + splitFld[i].trim() + '", ';
	}
	x = x + (splitFld[(splitFld.length - 1)]).trim() + ': data[i].' + splitFld[i].trim() + '\n';
	arr = arr + '"' + (splitFld[(splitFld.length - 1)]).trim() + '"';
	str = str + x;
	str = str + '});\n';
	str = str + '}\n';
	...
	str = str + 'BackupRestore.Backup(' + titleCase(tbl) + 'Model, ["_id", ' + arr + '], "' + tbl + '");\n';
	...
	str = str + 'router.get("/restore", (req, res) => {\n';
	str = str + '// BackupRestore.Restore('+ titleCase(tbl) +'Model, "src/backup/2022-02-15-'+tbl+'.js");\n';
	str = str + 'res.send("restored");\n';
	str = str + '});\n';
