...
const EditPage = (tbl, fld) => {
  ...
  str = str + 'import { Pencil } from "react-bootstrap-icons";\n';
  str = str + '\n';
  str = str + '\n';
  ...
  str = str + 'let getOneArray = jsonData.filter((t) => {\n';
  str = str + 'return t.id === id;\n';
  str = str + '});\n';
  str = str + 'let getOne = getOneArray[0];\n';

  let b = "";
  for (var j = 0; j < split_fld.length; j++) {
    if (j != 0) {
      b = b + 'set' + titleCase(split_fld[j].trim()) + '(getOne.' + split_fld[j].trim() + ');\n';
    }
  }
  str = str + b;
  ...
  str = str + 'let localData = localStorage.getItem("' + tbl + '");\n';
  str = str + 'let jsonData = JSON.parse(localData);\n';
  str = str + 'for (let i = 0; i < jsonData.length; i++) {\n';
  str = str + 'if (jsonData[i].id === id) {\n';
  str = str + 'jsonData[i] = obj;\n';
  str = str + '}\n';
  str = str + '}\n';
  str = str + 'localStorage.setItem("' + tbl + '", JSON.stringify(jsonData));\n';
  str = str + 'Message("Data updated successfully.");\n';
  str = str + 'setMainModalShow(false);\n';
  str = str + '}\n';
  str = str + '\n';
  str = str + '\n';
  ...
  str = str + '</Modal.Footer>\n';
  str = str + '</Modal>\n';
  str = str + '<Button size="sm" variant="info me-2" onClick={showModal} title="Edit"><Pencil size={15} /></Button>\n';
  str = str + '</>\n';
  str = str + ');\n';
  str = str + '\n';
  str = str + '\n';
