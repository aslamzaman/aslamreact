const LocalDatabase =()=>{
let errCode = "`Error editing item with id ${itemId} in ${key}: ${error}`";
...
export const editItem = (key, itemId) => {
  try {
      const data = retrieveData(key);
      const item = data.find(t => t.id === itemId);
      return item || null;
  } catch (error) {
...
export const updateItem = (key, itemId, updatedItem) => {
  try {
      const data = retrieveData(key);
      const indexOfItem = data.findIndex(t => t.id === itemId);
      if (indexOfItem === -1) {
