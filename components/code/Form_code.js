const FormCode = ()=>{
const btnCSS = "`text-center mt-3 mx-0.5 px-4 py-2 font-semibold rounded-md focus:ring-1 ring-blue-200 ring-offset-2 duration-300 ${Class} cursor-pointer`";
const btnsmCss = "`text-center mt-3 mx-0.5 px-4 py-2 text-sm font-semibold rounded-md focus:ring-1 ring-blue-200 ring-offset-2 duration-300 text-white ${Class}`";
const submitCss = "`text-center mt-3 mx-0.5 px-4 py-2 font-semibold rounded-md focus:ring-1 ring-blue-200 ring-offset-2 duration-300 ${Class}`";
const inputFileCss="`text-xs font-semibold mb-1 ${Class}`";


  let str = `export const BtnEn = ({ Title, Click, Class }) => {
    return (
      <input type="button" onClick={Click} className={${btnCSS}} value={Title} />
    )
  }

  export const BtnEnSm = ({ Title, Click, Class }) => {
    return (
      <button onClick={Click} className={${btnsmCss}}>{Title}</button>
    )
  }

  export const BtnSubmit = ({ Title, Class }) => {
    return (
      <button type="submit" className={${submitCss}}>{Title}</button>
    )
  }
 ...
 export const TextBn = ({ Title, Id, Change, Value, Chr }) => {
  return (
    <div className="w-full flex flex-col items-start">
      <label className='text-xs font-semibold mb-1' htmlFor={Id}>{Title}</label>
      <input onChange={Change} value={Value} type="text" id={Id} name={Id} required  maxLength={Chr} className="w-full px-4 py-1.5 font-SutonnyMJ_Regular text-gray-600 ring-1 focus:ring-4 ring-blue-300 outline-none rounded duration-300" />
    </div>
  )
}
...
export const TextEnDisabled = ({ Title, Id, Change, Value, Chr }) => {
  return (
    <div className="w-full flex flex-col items-start">
      <label className='text-xs font-semibold mb-1' htmlFor={Id}>{Title}</label>
      <input onChange={Change} value={Value} type="text" id={Id} name={Id} required maxLength={Chr} disabled className="w-full px-4 py-1.5 text-gray-600 ring-1 focus:ring-4 ring-blue-300 outline-none rounded duration-300" />
    </div>
  )
}
...
export const TextFile = ({ Title, Id, Change, Accept, Class }) => {
  return (
    <div className="w-full flex flex-col items-start">
      <label className={inputFileCss} htmlFor={Id}>{Title}</label>
...
export const DropdownBn = ({ children, Title, Id, Change, Value }) => {
  return (
    <div className="w-full flex flex-col items-start">
      <label className='text-xs font-semibold mb-1' htmlFor={Id}>{Title}</label>
      <select onChange={Change} value={Value} id={Id} name={Id} required className="w-full px-4 py-1.5  font-SutonnyMJ_Regular text-gray-600 ring-1 focus:ring-4 ring-blue-300 outline-none rounded duration-300">
        {children}
      </select>
    </div>
  )
...
export const TextareaEn = ({ Title, Id, Rows, Change, Value }) => {
  return (
    <div className="w-full flex flex-col items-start">
      <label className='text-xs font-semibold mb-1' htmlFor={Id}>{Title}</label>
      <textarea rows={Rows} id={Id} name={Id} onChange={Change} value={Value} required  maxLength="200"  className="w-full px-4 py-1.5 text-gray-600 ring-1 focus:ring-4 ring-blue-300 outline-none rounded duration-300" />
    </div>
  )
}
...
export const TextareaBn = ({ Title, Id, Rows, Change, Value }) => {
  return (
    <div className="w-full flex flex-col items-start">
      <label className='text-xs font-semibold mb-1' htmlFor={Id}>{Title}</label>
      <textarea rows={Rows} onChange={Change} value={Value} id={Id} name={Id} maxLength="400" className="w-full px-4 py-1.5 font-SutonnyMJ_Regular text-gray-600 ring-1 focus:ring-4 ring-blue-300 outline-none rounded duration-300" />
    </div>
  )
}
