...
const Edit = ({ Msg, Id }) => {
    ...
    const editHandler = async () => {
        ...
        Msg("Ready to edit");
        try {
            const [unitsalaryData, staffData] = await Promise.all([fetchOne("unitsalary", Id), fetchAll("staff")]);

            const fieldStaff = staffData.filter(s=> parseInt(s.place_id)===1699884054697);
            setStaffs(fieldStaff ? fieldStaff : []);

            if (unitsalaryData) {
                const { staff_id, arear, sal1, sal2, remarks } = unitsalaryData;
            ...
            console.log(`Error fetching unitsalary data: ${error}`);
        }
    };


    const createUnitsalaryData = () => {
        return {
            id: Id,
            staff_id: staff_id,
            arear: arear,
    ...
    const saveHandler = async (e) => {
        e.preventDefault();
        try {
            const unitsalaryData = createUnitsalaryData();
            const updatedUnitsalaryId = await updateOne("unitsalary", unitsalaryData);
            console.log(`Unitsalary with id ${updatedUnitsalaryId} updated successfully.`);
            Msg("Data updated successfully.");
        } catch (error) {
            console.log(`Error updating unitsalary data: ${error}`);
