...
const Edit = ({ Msg, Id }) => {
    ...
    const editHandler = async () => {
        ...
        console
        try {
            const unitData = await fetchOne("unit", Id);
            if (unitData) {
                const { nm_en, nm_bn } = unitData;
                setNm_en(nm_en);
                setNm_bn(nm_bn);
            } else {
                setNm_en("");
    ...
    const saveHandler = async (e) => {
        ...
        try {
            ...
            const updatedUnitId = await updateOne("unit", unitData);
            console.log(`Unit with id ${updatedUnitId} updated successfully.`);
            Msg("Data updated successfully.");
        } catch (error) {
            console.log(`Error updating unit data: ${error}`);
                    ...
                    <div className="px-6 pb-6 text-black">
                        <form onSubmit={saveHandler} >
                            <div className="grid grid-cols-1 gap-4 my-4">
                                <TextEn Title="English Name" Id="nm_en" Change={e => setNm_en(e.target.value)} Value={nm_en} Chr="50" />
                                <TextBn Title="Bangla Name" Id="nm_bn" Change={e => setNm_bn(e.target.value)} Value={nm_bn} Chr="50" />
                            </div>
