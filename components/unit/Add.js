...
const Add = ({ Msg }) => {
    ...
    const saveHandler = async (e) => {
        e.preventDefault();
        try {
            const newUnitData = createUnitObject();
            await insertOne("unit", newUnitData);
            Msg("Data saved successfully.");
        } catch (error) {
            console.log(`Error saving unit data: ${error}`);
            Msg("Error saving unit data.");
        }
        setShow(false);
    }
