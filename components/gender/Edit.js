...
const Edit = ({ Msg, Id }) => {
	...
	const editHandler = async () => {
		...
		Msg("Ready to edit");
		try {
			const genderData = await fetchOne("gender", Id);
			if (genderData) {
	...
	const saveHandler = async (e) => {
		e.preventDefault();
		try {
			const genderData = createGenderData();
			const updatedGenderId = await updateOne("gender", genderData);
			console.log(`Gender with id ${updatedGenderId} updated successfully.`);
			Msg("Data updated successfully.");
		} catch (error) {
			console.log(`Error updating gender data: ${error}`);
