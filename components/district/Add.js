...
const Add = ({ Msg }) => {
	...
	const saveHandler = async (e) => {
		e.preventDefault();
		try {
			const newDistrictData = createDistrictObject();
			await insertOne("district", newDistrictData);
			Msg("Data saved successfully.");
		} catch (error) {
			console.log(`Error saving district data: ${error}`);
			Msg("Error saving district data.");
		}
		setShow(false);
	}
