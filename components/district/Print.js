...
const district = await fetchAll("district");

let y = 20;
for(let i = 0; i < district.length;i++){
doc.text(`${district[i].name}`, 30, y , null, null, "center");
doc.text(`${district[i].bn_name}`, 60, y , null, null, "center");
y = y + 6;
}

doc.save(`${Date.now()}-district.pdf`);
Msg("Print completed.");
setShow(false);
}
