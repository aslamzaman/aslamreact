...
import { BtnEn, TextDt, DropdownEn } from "../../components/Form";
...
const Print = ({ Msg }) => {
    ...
    const printShow = () => {
        ...
        const getData = async () => {
            try {
                const [staffs, deg] = await Promise.all([fetchAll("staff"), fetchAll("post")]);

                const joinStaff = staffs.map(s => {
                    const matchPost = deg.find(d => parseInt(d.id) === parseInt(s.post_id));
                    return {
                        ...s,
    ...
    const printHandler = () => {
        ...
        setTimeout(() => {
            ...
            doc.save(`${new Date().toISOString()}-bkash-bill.pdf`);            
        }, 0);
        setShow(false);
    }
